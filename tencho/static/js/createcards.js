  ...
  $.ajax({
    ...
    success: function(data) {
        ...
        for (let key in data){
            ...
            out+='<p class="price">Цена: '+data[key].cost+'₽</p>'
            out+='<a class="slider-buy-btn" href="/product_detail/'+data[key].id+'" onclick="buy(this)" data-name="'+data[key].name+'" data-cost="'+data[key].cost+'"  data-img="'+data[key].image+'" data-desc="'+data[key].description+'" data-category="'+data[key].category+'">Купить</a>'
            out+='</div>'
            out+='</div>'
            out+='</div>'
      }
